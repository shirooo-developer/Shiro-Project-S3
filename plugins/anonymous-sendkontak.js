...
async function handler(m, { command, conn, text }) {
	...
	if (!room) throw '*Kamu Tidak Berada Dianonymous Chat*'
...
Nama: *${name}*`
	...
	if (other) this.sendHydrated(other, `${htki} ᴀɴᴏɴʏᴍᴏᴜs ᴄʜᴀᴛs ${htka}`, tks, await conn.profilePictureUrl(m.sender, 'image').catch(_ => './src/avatar_contact.png'), `wa.me/${m.sender.split`@`[0]}`, 'ᴛᴜʀɴ ᴄʜᴀᴛ ᴘᴀʀᴛɴᴇʀ', null,null, [['𝗟𝗘𝗔𝗩𝗘', '.leave'],[null,null],[null,null]], 0,  { mentionedJid: [m.sender]})
}
