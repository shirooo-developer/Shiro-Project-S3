...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `*Fitur Untuk Unduh Story IG*\n\n*_Contoh: ${usedPrefix + command} katyhusaclara_*`
    if (args[0].startsWith('http') || args[0].startsWith('@')) throw `*_User Name Tidak Ditemukan_*`
    try {
    ...
    conn.sendMedia(m.chat, url, null, { mentions: [m.sender], jpegThumbnail: await(await fetch(preview)).buffer(), caption: `*🌐 Link:* ${await(await axios.get(`https://tinyurl.com/api-create.php?url=${url}`)).data}`})
        ...
        throw `*_Dia Tidak Membuat Story_*`
    }                  
  }
}
