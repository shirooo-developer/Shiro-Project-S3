...
async function handler(m, { conn, args }) {
    ...
    if (m.sender in confirm) throw '*Tunggu Sebentar, Kamu Sedang Judi*'
    try {
        let user = global.db.data.users[m.sender]
        let count = (args[0] && number(parseInt(args[0])) ? Math.max(parseInt(args[0]), 1) : /all/i.test(args[0]) ? Math.floor(parseInt(user.money)) : 1) * 1
        if ((user.money * 1) < count) return m.reply('*_💵 Money Kamu Tidak Cukup_*')
        if (!(m.sender in confirm)) {
            confirm[m.sender] = {
                sender: m.sender,
                count,
                timeout: setTimeout(() => (m.reply('*_Timed Out_*'), delete confirm[m.sender]), 60000)
            }
            let txt = `*Yakin Mau Melakukan Judi?*\n\nKetik *Y* Untuk Yakin\nKetik *N* Untuk Tidak\n\n*Taruhan:* ${count} 💵\n*⏰ 60s Timeout*`
...
Kamu *${status}*, Kamu ${status == 'Menang' ? `Mendapatkan *+${count * 2} Money 💵*` : status == 'Kalah' ? `Kehilangan *-${count * 1} Money 💵*` : `Mendapatkan *+${Math.floor(count / 1.5)}*`} Money 💵
    `.trim())
            clearTimeout(timeout)
            delete confirm[m.sender]
            return !0
