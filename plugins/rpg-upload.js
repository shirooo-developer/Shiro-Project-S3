...
let handler = async (m, { conn, participants, usedPrefix, owner }) => { 
    ...
    if (!m.isGroup) {
        ...
        if (global.db.data.users[m.sender].stamina > 99) {
            ...
            let _stamina = `${Math.floor(Math.random() * 199 * buff1)}`.trim()
            let stamina = (_stamina * 1)
            let likee = `${Math.floor(Math.random() * 20 * buff1 * buf2)}`.trim() 
            let dislikee = `${Math.floor(Math.random() * 20 * buff1 * buf2)}`.trim() 
            let viewerr = `${Math.floor(Math.random() * 30 * buff1 * buf2)}`.trim() 
            let subscriberr = `${Math.floor(Math.random() * 10 * buff1)}`.trim() 
