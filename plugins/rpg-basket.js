...
let buff1 = (buf1 == 0 ? '0' : '' || buf1 == 1 ? '2' : '' || buf1 == 2 ? '3' : '' || buf1 == 3 ? '4' : '' || buf1 == 4 ? '5' : '' || buf1 == 5 ? '6' : '' || buf1 == 6 ? '35' : '' || buf1 == 7 ? '40' : '' || buf1 == 8 ? '45' : '' || buf1 == 9 ? '50' : '' || buf1 == 10 ? '100' : '')
    ...
    if (!m.isGroup) {
        ...
        if (global.db.data.users[m.sender].drink > 9) {
            ...
            let drink = (_drink * 1)
            let staminaa = `${Math.floor(Math.random() * 90)}`.trim() 
            let str = `
...
`.trim()          
            ...
            setTimeout(() => {
                   ...
                   m.reply(`*Kamu Bertanding Bola Basket Dengannya*`)
            ...
            setTimeout(() => {
                   ...
                   m.reply(`*${pickRandom(['Mario Wuysang', 'Arki Dikania Wisnu', 'Abraham Damar Grahita', 'Rony Gunawan', 'Senjata Galank', 'Kristen Ronaldo Sitepu', 'Kelly Purwanto', 'Dimaz Muharri','Hardianus Lukudu','Vincent Kosasih'])} Akan Bertanding Denganmu*`)
...
handler.command = /^(basket)$/i

handler.limit = 1

handler.skillsport = 9
