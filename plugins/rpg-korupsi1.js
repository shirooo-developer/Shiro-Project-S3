...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 72000000) {
      if (Aku > Kamu) {
        conn.sendFile( m.chat, kbansos, 'b.jpg', `*Kamu Tertangkap Karena Korupsi Dan Kamu Didenda Sebesar 3000000 💵*`, m)
        user.money -= 6000000
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 6000000
        conn.sendFile( m.chat, mbansos, 'b.jpg', `*Kamu Berhasil Korupsi Dan Kamu Mendapatkan Money Sebesar 3000000 💵*`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else {
