...
let handler = async (m, { conn, participants, usedPrefix, owner }) => { 
    ...
    if (!m.isGroup) {
        ...
        if (1 > users[who].tipeyt) throw '*Belum Memilih Tipe YouTuber Di #youtuber & Nama Channel Di #chname*'
        let timers = clockString(_timers)

        if (global.db.data.users[m.sender].stamina > 99) {

            if (new Date - global.db.data.users[m.sender].lastup > 7200000) {


            let _stamina = `${Math.floor(Math.random() * 199 * buff)}`.trim()
            let stamina = (_stamina * 1)
            let likee = `${Math.floor(Math.random() * 90 * buff1 * buf2)}`.trim() 
            let dislikee = `${Math.floor(Math.random() * 90 * buff1 * buf2)}`.trim() 
            let viewerr = `${Math.floor(Math.random() * 200 * buff1 * buf2)}`.trim() 
            let subscriberr = `${Math.floor(Math.random() * 40 * buff1)}`.trim() 
            let showtimee = `${Math.floor(Math.random() * 3 * buff1)}`.trim() 
            let moneyy = `${Math.floor(Math.random() * 50 * buff * buff1 * buf2)}`.trim() 
            let videoo = `${1}`.trim() 
            let str = `
*UPLOADING VIDEOS*
