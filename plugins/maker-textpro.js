...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command }) => {
  if (!effect) throw '*Fitur Membuat Text Jpg*\n\n*_Format: .textpro <nama> <efek>_*\n*_Contoh: .textpro wood iya_*\n\n╭─❑ 「 *LIST EFFECT* 」\n' + effects.map(v => v.title).join('\n│ • ')
...
let name = await conn.getName(who)
 let stiker = await sticker(null, global.API(`https://telegra.ph/file/8fbc2359ca0cd74a0f4ca.png`), global.packname, global.author)
 const fliveLoc2 = {
            key:
            { fromMe: false,
            participant: `0@s.whatsapp.net`, ...(m.chat ? 
            { remoteJid: "status@broadcast" } : {}) },
            message: { "liveLocationMessage": { "title": "© Created By么 Kitsuneee","⌬ Felicia-MD": wm, 'jpegThumbnail': fs.readFileSync('./thumbnail.jpg')}}
           }
 ...
 conn.send3ButtonImg(m.chat, result.data, `Ini Kak *${tag}*\nDone Creating Efek *${effect}*`,`${global.sgc}`, '𝗠𝗘𝗡𝗨', '.menu', '𝗢𝗪𝗡𝗘𝗥', '.owner', '𝗖𝗥𝗘𝗗𝗜𝗧', '.credit', global.fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: sgc,
    mediaType: 2, 
    description: sgc,
    title: "Done Creating",
    body: wm,
    thumbnail: fs.readFileSync('./thumbnail.jpg'),
    sourceUrl: `https://wa.me/6281347927862?text=Hai+Ayg`
     }}
  })
}
