...
let buff1 = (buf1 == 0 ? '0' : '' || buf1 == 1 ? '2' : '' || buf1 == 2 ? '3' : '' || buf1 == 3 ? '4' : '' || buf1 == 4 ? '5' : '' || buf1 == 5 ? '6' : '' || buf1 == 6 ? '35' : '' || buf1 == 7 ? '40' : '' || buf1 == 8 ? '45' : '' || buf1 == 9 ? '50' : '' || buf1 == 10 ? '100' : '')
    ...
    if (!m.isGroup) {
        ...
        if (global.db.data.users[m.sender].drink > 9) {
            ...
            let drink = (_drink * 1)
            let staminaa = `${Math.floor(Math.random() * 40)}`.trim() 
            let str = `
...
`.trim()          
            ...
            setTimeout(() => {
                   ...
                   m.reply(`*Kamu Bertanding Golf Dengannya*`)
            ...
            setTimeout(() => {
                   ...
                   m.reply(`*${pickRandom(['Rory Hie', 'George Gandranata', 'Danny Masrin', 'Andik Mauludin', 'Yosua Andrew Wirawan', 'Kevin C. Wirawan', 'Almay Rayhan Yaqut', 'Ian Andrew','Sean Riyan','Kurnia Heriawan'])} Akan Bertanding Denganmu*`)
...
handler.command = /^(golf)$/i

handler.limit = 1

handler.skillsport = 4
