      ...
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          ...
          if (user.boostb > 4) return m.reply('*_Level Boost Kamu Sudah Max_*')
            if(user.money < 59 * buff2 || user.gems < 39999 * buff2) return m.reply(`*Kamu Membutuhkan*\n• Gems ${59 * buff2} 💠\n• Money ${39999 * buff2} 💵`)
          ...
          if (user.boosts> 4) return m.reply('*_Level Boost Kamu Sudah Max_*')
            if(user.money < 59 * buff3 || user.gems < 39999 * buff3) return m.reply(`*Kamu Membutuhkan*\n• Gems ${59 * buff3} 💠\n• Money ${39999 * buff3} 💵`)
